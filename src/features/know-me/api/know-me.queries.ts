 ...
 */
export function usePartnerSubmitted(dayId: string | undefined) {
  const userId = useUserId();
  return useQuery({
...
/**
 * History from the `know_me_revealed` view ONLY (both-submitted days). Shaped
 * into `DayRecord[]` from the current user's perspective.
 */
export function useHistory() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<DayRecord[]> => {
      ...
      if (error) throw error;
      const rows = (data ?? []) as unknown as RevealedRow[];
      ...
      return records.sort((a, b) => b.coupleDay.localeCompare(a.coupleDay));
    },
  });
}

/** The love-map, derived from history. */
export function useLoveMap(): LoveMap | undefined {
  const { data } = useHistory();
  return data ? buildLoveMap(data) : undefined;
}
...
export function useHistoryEntries() {
  const userId = useUserId();
  return useQuery({
    queryKey: [...qk.knowMe.history(), 'entries'],
    enabled: !!userId,
    queryFn: async (): Promise<HistoryEntry[]> => {
      ...
      return entries.sort((a, b) => b.coupleDay.localeCompare(a.coupleDay));
    },
  });
}
