...
--
-- IDEMPOTENT: only inserts prompts not already present, so re-running (db reset,
-- a re-push after the rows were applied out-of-band, etc.) never duplicates.
-- ════════════════════════════════════════════════════════════════════════

insert into public.know_me_questions (prompt, category, options)
select v.prompt, v.category, v.options::jsonb
from (
  values
...
) as v(prompt, category, options)
where not exists (
  select 1 from public.know_me_questions q where q.prompt = v.prompt
);
